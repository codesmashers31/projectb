...
export const MentorJobCard = React.memo(({ mentor }: { mentor: MentorProfile }) => {
    ...
    const cleanPrice = `₹${priceNum.toLocaleString("en-IN")}`;

    // Mockup strikethrough price and percentage to match the exact mockup cases
    let originalPriceVal = 1299;
    let discountPercent = 23;
    ...
    return (
      ...
      >
        ...
        <div className="w-full pt-5 flex flex-col items-center">
          ...
          {/* Skills pills row */}
          <div className="flex items-center justify-center gap-1.5 mb-3.5 flex-wrap w-full px-1">
            {(mentor.skills || []).slice(0, 3).map((skill) => (
              <span
                key={skill}
                className="text-[10px] font-bold px-2.5 py-0.5 bg-[#EEF2FF] text-[#4F46E5] rounded-full whitespace-nowrap"
              >
                {skill}
              </span>
            ))}
        ...
        <div className="w-full flex flex-col items-center">
          ...
          <div className="text-left w-full mb-2.5 px-1">
            ...
            <div className="flex items-baseline gap-2 flex-wrap">
              <span className="text-xl font-extrabold text-gray-900">{cleanPrice}</span>
              <span className="text-xs text-gray-400 line-through font-medium">{originalPrice}</span>
              <span className="text-[9px] font-bold px-2 py-0.5 bg-[#FEE2E2] text-[#DC2626] rounded-md">
                {discountPercent}% OFF
              </span>
            </div>
          </div>
          ...
          {/* Green Next Available Bar */}
          <div className="w-full bg-[#f0fdf4] border border-[#dcfce7]/60 rounded-xl py-2 px-3 flex items-center justify-center gap-1.5 text-xs font-semibold text-[#166534] shadow-sm">
            <CalendarIcon />
            <span className="truncate">Next Available: {mentor.activeTime || "Today, 6:30 PM"}</span>
          </div>
        </div>
      </div>
    );
